    ...
    def l1cProjtoa(self, lat, lon, toa, band):
        ...
        for i in range(lat.shape[0]):
            for j in range(lon.shape[1]):
                mg_mgrs = str(m.toMGRS(lat[i, j], lon[i, j], inDegrees=True, MGRSPrecision=self.l1cConfig.mgrs_tile_precision))
                mgrs_tiles.add(mg_mgrs)

        mgrs_tiles = list(mgrs_tiles)
        mgrs_tiles_len = len(mgrs_tiles)
        lat_l1c = np.zeros(mgrs_tiles_len); lon_l1c = np.zeros(mgrs_tiles_len); toa_l1c = np.zeros(mgrs_tiles_len)
        for i in range(mgrs_tiles_len):
            lat_l1c[i], lon_l1c[i] = m.toLatLon(mgrs_tiles[i], inDegrees=True)
